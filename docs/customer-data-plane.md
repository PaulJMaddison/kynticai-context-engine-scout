...
This page explains how Context Engine · Scout runs beside customer systems as a customer-controlled data plane for exact data items, relationships, attribution paths, outcomes, governed context, and local APIs.
...
These records are carried as exact linked records with citation IDs. Deterministic relationships such as email-to-contact, contact-to-account, account-to-opportunity, account/contact-to-activity, contact-to-email-engagement, account/contact-to-web-conversion, account/contact-to-support-ticket, account/contact-to-product-usage, account-to-billing, and account/contact-to-outcome can be demonstrated locally. Attribution paths record what happened and in what order. The fictional `examples/KynticAI.Scout.Reference.Sales` consumer may apply `BasicRelationshipEngine` fallback weights and construct a recommended next action for demo/reference purposes; Scout production/core DI does not register that behaviour. Advanced relationship-set analysis, attribution-path comparison, outcome matching, and governed analysis belong to Fortress/private extensions. Optional Cloud/control-plane output remains aggregate usage metadata only: counters, status, timestamps, package version, and audit/control-plane event metadata, not relationship types, attribution paths, weights, recommendations, confidence, caveats, citation IDs, raw operational records, or per-entity relationship metadata.
...
7. Downstream consumers retrieve governed context through REST, GraphQL, SDKs, internal services, or context packages.
8. Reference/customer-owned/private consumers may then perform next-action, recommendation, workflow or model work. The legacy `/api/v1/intelligence/next-action` core endpoint remains only as an explicit `501 feature.external_consumer_required` compatibility surface.
