...
workflows=()
while IFS= read -r workflow; do workflows+=("$workflow"); done < <(find .github/workflows -maxdepth 1 -type f \( -name '*.yml' -o -name '*.yaml' \) 2>/dev/null | sort)
if [[ ${#workflows[@]} -eq 0 ]]; then
  echo "No active GitHub Actions workflow files found; CI/CD is currently disabled (workflows are renamed to .disabled)."
else
  workflow_forbidden=(
    '\$\{\{\s*secrets\.'
    '-----BEGIN'
